    ...
    fs.readdir(path, function (err, files) {
        ...
        files.forEach(function (fileName, index) {
            fs.stat(path + '/' + fileName, function (err, stat) {
                ...
                if (err) {
                    result = self.sendError_(req, res, err);
                }
                if (stat.isDirectory()) {
                    files[index] = fileName + '/';
                }
                if (!(--remaining)) {
                    result = self.writeDirectoryIndex_(req, res, path, files);
                }
...
function mkFile (fileDirAndName, data, callback) {
    fs.exists(path.dirname(fileDirAndName), function (dirExists) {
        if (!dirExists) {
            fs.mkdirSyncRecursive(path.dirname(fileDirAndName));
        }

        fs.writeFile(fileDirAndName, data, callback);
    });
}

function processUsers (userDataFileDir) {
    console.log('Creating user files from \"' + userDataFileDir + '\"...');

    fs.stat(userDataFileDir, function (err, stats) {
        if (stats.isFile()) {
            fs.readFile(userDataFileDir, function (err, json) {
                var arr = parseJson(err, json),
                    count = 0,
                    callback = function (err) {
                        if (err) {
                            ...
                            if (count === arr.length) {
                                console.log(count + ' user files created successfully');
                            }
                        }
                    };
                ...
                for (var i = 0; i < arr.length; i++) {
                    if (arr[i].id) {
                        mkFile('./data/users/' + arr[i].id + '.json', JSON.stringify(arr[i]), callback);
                    }
                }
            });
        }
...
function processLogs (logDataFileDir) {
    console.log('Creating user files from \"' + logDataFileDir + '\"...');

    fs.stat(logDataFileDir, function (err, stats) {
        if (err) {
            throw err;
        } else if (stats.isFile()) {
            fs.readFile(logDataFileDir, function (err, json) {
                var arr = parseJson(err, json),
                    processLog = function (log) {
                        if (log.user_id) {
                            var filePath = './data/logs/' + log.user_id + '.json';

                            if (!fs.existsSync(path.dirname(filePath))) {
                                fs.mkdirSyncRecursive(path.dirname(filePath));
                            }

                            var fileExists = fs.fileExistsSync(filePath);

                            fs.appendFileSync(filePath, (fileExists ? ',' : '') + JSON.stringify(log));
                        }
                    };
                ...
                processLogFiles('./data/logs/');
            });
        } else {
            throw logDataFileDir + ' could not be found.';
        }
    });
}

function addJsonArray (file, contents) {
...
function processLogFiles (dirPath) {
        ...
        files.filter(function (file) {
            return path.extname(file) === '.json';
                ...
                fs.readFile(filePath, 'utf-8', function (err, contents) {
                    ...
                    addJsonArray(filePath, contents);
                    count++;

                    if (count === files.length) {
                        console.log(count + ' log files created successfully');
                    }
                });
            });
    });
...
// Must be last,
main(process.argv);
//processUsers('./data/users.json');
//processLogs('./data/logs.json');